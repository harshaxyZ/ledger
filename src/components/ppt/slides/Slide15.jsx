    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full max-w-[1200px] flex justify-between items-center text-left">
        <motion.div initial={{ opacity: 0, y: 80 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="w-[55%] flex justify-center">
          <img src="/image_4.png" alt="Screenshot" className="w-full max-h-[60vh] object-contain rounded-[20px] border-[2px] border-[rgba(255,255,255,0.1)] shadow-2xl" />
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="w-[40%] bg-[rgba(10,10,10,0.85)] backdrop-blur-[10px] border border-[rgba(255,255,255,0.08)] p-[40px] rounded-[16px]">
          <h3 className="text-[32px] font-bold text-[#ffffff] mb-[24px]">Visual Breakdown</h3>
          <p className="text-[24px] text-[#d0d0d0] leading-[1.7] whitespace-pre-line">Transforms raw data into actionable intelligence.

Users see total spending, transaction count, average per transaction, and top category visually charted out for clarity.</p>
        </motion.div>
      </div>
    </div>
  );
